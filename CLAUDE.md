...
pnpm run extract:item-codes  # Refresh src/data/item-codes.ts (stable APPEND-ONLY item→URL-code registry) — STANDALONE + self-contained (parses committed constants.ts, no data dir); re-run after extract:items
...
- `src/data/item-codes.ts` + `src/lib/item-code.ts` — AUTO-GENERATED (`extract:item-codes`) **stable, APPEND-ONLY** item→URL-code registry (code = base36 array index) + the `encodeItemRef`/`decodeItemRef` helpers that shorten item ids in the URL (`t`/`r`/`m` plan params + the `s=` settings `R` field). **Codes must never change or be reused** — shared URLs reference them — so the generator only appends new items + tombstones (`""`) removed ones. `decodeItemRef` also accepts full ids (legacy-link back-compat). The `item-code.test.ts` completeness guard fails if a new item lacks a code.

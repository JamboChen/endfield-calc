...
pnpm run extract:url-codes   # Refresh src/data/{item,recipe,facility,structure}-codes.ts (stable APPEND-ONLY id→URL-code registries) — STANDALONE + self-contained (parses committed constants.ts, no data dir); re-run after extract:all
...
- `src/data/{item,recipe,facility,structure}-codes.ts` + `src/lib/url-codes.ts` — AUTO-GENERATED (`extract:url-codes`) **stable, APPEND-ONLY** id→URL-code registries (code = base36 array index) + the `makeRefCodec`-built `encode*Ref`/`decode*Ref` helpers that shorten ids in the URL: items (`t`/`r`/`m` plan params + the `s=` settings `R` field), recipes (`r`), facilities (`s=`'s `A` field), structures (`s=`'s `S` field). **Codes must never change or be reused** — shared URLs reference them — so the generator only appends new ids + tombstones (`""`) removed ones. Codes are lowercase base36, which keeps the settings blob's uppercase-letter field delimiter unambiguous. `decode*Ref` also accepts full ids (legacy-link back-compat) and returns `null` for ids absent at runtime, so it doubles as the existence check. The `url-codes.test.ts` completeness guard fails if a new id lacks a code.

...
- `src/data/gas-sustain.ts` — AUTO-GENERATED (`extract:sustain`) 1.4 gas-sustain model: `facilitySustainDrains` (transmuter catalysts, 6/min per WHOLE building even when idle) + `vaporizerEnvs` (env id → gas + synthetic zero-output `vaporize_*` recipe). ALWAYS ACTIVE in the calculator (unlike `powerSustain`): catalyst folds into transmuter recipe clones at graph build, the unified ceil-floor loop scales it PER RECIPE to `rate × ceil(fc_r)` (plan surfaces the decomposition as the `ProductionGraphNode.catalyst` contract — display code reads that, never game data) and forces whole always-on vaporizers per `Recipe.gasEnv` group via `LPInput.recipeMinRates`. Coverage ratio = `machinesPerVaporizer` plan option (URL `mpv`, default 4 from `src/lib/sustain-constants.ts` — UI-safe, keeps the solver code-split). See `.claude/rules/solver.md`.

...
- `facilityRecipeVariants: ReadonlyMap<FacilityId, { default; toggled }>` — per-facility recipe-variant pairs gated by a structure toggle. Today: `SEWAGE_INLET` → `{ default: SEWAGE_INLET_DISPOSAL, toggled: SEWAGE_INLET_BYPRODUCT }`. Drives the App-side variant filter + the calculator-side defensive filter; see `.claude/rules/domain-settings.md` for the bridge and `.claude/rules/solver.md` for how the LP picks the active variant.
...
- Synthetic facility `sewage_inlet` has no own PNG and falls back to `liquid_cleaner_1.png` via `FACILITY_ICON_FALLBACK` in `src/lib/facility-icons.ts`.

## Generated vs manual facilities

`src/data/facilities.ts` is fully auto-generated by `scripts/build-facilities.ts` from upstream `FactoryBuildingTable.json`. **Hand edits will be clobbered on the next `bun run extract:facilities`.** Synthetic facilities that don't exist in upstream game data (today: `SEWAGE_INLET`, modeling the Wuling map-structure inlets as one capped facility) live in `src/data/manual-facilities.ts`. The `@/data` barrel concatenates them: `facilities: Facility[] = [...generated, ...manual]`. Consumers MUST import `facilities` from `@/data`, not from `@/data/facilities`, or they'll miss the synthetic entries.

...
paths:
  ...
  - "src/data/manual-facilities.ts"
  - "src/data/recipes.ts"
  - "src/data/region-structures.ts"
...
`src/data/facilities.ts` is fully auto-generated by `scripts/build-facilities.ts` from upstream `FactoryBuildingTable.json`. **Hand edits will be clobbered the next time the facility-extraction script is run.** Synthetic facilities that don't exist in upstream game data (today: `LIQUID_CLEAN_GATE_1`, modeling the three Wuling sewage inlet gates as one capped facility) live in `src/data/manual-facilities.ts`.

The `@/data` barrel concatenates them with a manual-wins dedup filter (`facilities = [...generated.filter(f => !manualIds.has(f.id)), ...manual]`). Consumers MUST import `facilities` from `@/data`, not from `@/data/facilities`, or they'll miss the synthetic entries.

**Naming convention for manual entries**: use the canonical upstream game building ID verbatim. When a single LP-side facility collapses multiple in-game buildings (because they're functionally identical for solver purposes), name it after the tier-1 / first variant — mirrors how `liquid_cleaner_1` represents the Water Treatment Unit when game tiers 2/3 exist. The dedup filter is the structural backstop in case a future maintainer adds the canonical id to the build-facilities `GROUP_A_ALLOWLIST` (deliberately discouraged; see the `manual-facilities.ts` JSDoc).

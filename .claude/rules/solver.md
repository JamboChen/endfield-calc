...
## Producible raws (vent-mine vs. craft)

`producibleRaws` (`src/data/index.ts`) = `rawMaterialSources.keys() ∖ costlessRaws` — every raw EXCEPT the costless liquids (opt-out; today `{ores…, muck, item_gas_inert, item_gas_xiranite}`). A raw in this set that ALSO has a surviving producer under the current constraints (today only Xiragen via the transmuters, and Inergen via a jar-dismantle byproduct) is NOT modeled as an infinite leaf:

- **Graph** (`graph-builder.ts:traverse`): such a raw is recorded in `graph.producibleRaws` (disjoint from `graph.rawMaterials`) and its producers are added like a normal item. Its item node KEEPS `isRawMaterial: true` (still vent-sourced). Producer-less members (ores) fall through to the normal raw-leaf path — a harmless no-op.
- **LP** (`lp-solver.ts:buildModel`): a producible raw is a normal balance-constrained item (absent from `input.rawMaterials`, so it gets a `c_*` row and its consumers are auto-excluded from consumer-side `rawCost`). Its vent/mine draw is ONE `rawsupply_*` variable (`+1` on the balance row, `rawCost = RAW_SUPPLY_COST_PER_UNIT = 1`/unit — the SAME per-unit price the raw carried before, so nothing reroutes through it), bounded by the node cap (moved off the consumption-sum raw-cap row onto this variable; overflow → shared `rawCapSlackVarMap` → `rawCapOveruse`). Mirrors a Metastorage import, but cap-bounded not budget-bounded.
- **Consume-before-craft** is enforced on the CRAFT side: `producibleRawCraftSurcharge` adds `PRODUCIBLE_RAW_CRAFT_SURCHARGE` (= the vent's 1/unit) to the `rawCost` of any recipe whose SOLE output is a producible raw (the transmuters — byproduct emitters like the Inergen dismantler are exempt). A craft therefore always costs ≥ mining the same unit, so the LP fills the capped vent first and crafts only the OVERFLOW above the cap. **MUST be summed at both the model-build AND `extractSolution` `totalRaw` sites** or the rawCost lex-cap anchoring goes inconsistent.
- **Read-back**: `LPSolution.rawSupplyRates` → `FlowData.rawSupplyRates` → the item node's `rawSupplyRate` (vent draw only; `productionRate` = vent + craft). Pickup-pump sizing (`plan-helpers.aggregateBinTotals`) and the `raw-over-cap` judge (`computeLimitViolations`) read `rawSupplyRate ?? productionRate` so they bound the mined portion, not total.
- **Options + UI**: `CalculateProductionPlanOptions.producibleRaws` defaults to the `@/data` set; tests pass `new Set()` to restore pure-leaf behaviour (e.g. to keep a deliberately-deadlocked pinned cycle from gaining a craft escape). The Add-Target picker admits producible raws with a real recipe past its region-raw filter. Mappers render a producible-raw target as a sink fed by the vent pickup (`bin-fused-mapper.emitProducibleRawTargetSinks`; the merged mapper additionally splits craft overflow across the transmuter producers).
...
- **Auto-selection** lives in `calculator.ts:selectMetastorageImports`: sequential greedy per route; candidates = eligible ∩ graph balance-row items that are targeted or consumed, `ItemId`-sorted; one `calculateFlows` per candidate; lex comparison `compareSolveMetrics` (feasible → slackMagnitude → **facilityPlacementOveruse** → powerShortfall → rawCost → buildings → power → TTV, `METASTORAGE_METRIC_TOLERANCE` epsilons); strict improvement required so the no-import baseline wins ties.
- **`facilityPlacementOveruse` (`FlowSolveMetrics`, built in `flow-solver.ts:buildSolveMetrics`)** — `Σ` over capped SINGLE-FORMULA facilities of `max(0, Σ ceil(fc_r) − cap)`: the physical buildings a candidate would place OVER a facility cap through fragmentation. The LP's facility-cap slack only sees the FRACTIONAL total, so a cheaper import that runs a 0.42-building sliver of an extra recipe on a capped single-formula facility looks free to it — but that sliver ceils to a whole extra building (user-reported: `equip_script_4_3` @ 14.5 with a lowered Xiragen cap — importing `carbon_enr` split `xiranite_powder` across two Forge recipes → 13th Forge over cap 12, while no-import fit at 12). Ranked right after `slackMagnitude` (a building over a facility cap is an UNBUILDABLE hard limit) so **enabling Metastorage can only ADD supply, never worsen the cap verdict**. Its own comparison key (buildings, cross-unit rule); 0 when no facility caps are set. Mix pools consolidate recipes per bin so they're skipped (same scope as the cap-tightening in `calculator.ts`).
...
- **Cross-solve, the same lattice is re-stated structurally, never by summing units**: `powerShortfall` (watts) is its OWN key in `compareSolveMetrics` — `feasible → slackMagnitude → facilityPlacementOveruse → powerShortfall → rawCost → buildings → power → ttvUsed` — and is deliberately NOT folded into `slackMagnitude` (items/min scale). Folding it once let the Metastorage selection trade a 50 ore/min cap violation for a token 367 W of generation (user-reported: toggling power flipped the Valley IV route from originium powder to battery_3, ore 540→590). Any future soft tier gets its own comparison key at the position matching its LP penalty tier.
...
- **Env-coverage tie rows** (`LPInput.envCoverage`): HARD `envcov_*` rows `machinesPerVaporizer × x_vaporize − Σ env-machine vars ≥ 0`, built by the calculator for every injected vaporize recipe and passed to EVERY solve (baseline + metastorage candidates + loop re-solves). This is what makes an env route pay its gas share from the FIRST solve — without it, env recipes are free until the loop imposes min-runs, and min-run rows are unconditional, so the vaporizer gas becomes a sunk cost every re-solve ignores when choosing routes (env routes permanently underpriced). Pinned by the "pays its gas share from the FIRST solve" test in `gas-sustain.test.ts`.
- **Vaporizer min-runs**: `LPInput.recipeMinRates` — HARD `minrun_*` rows `x_r ≥ floor` (no slack; the floored recipes consume gas whose supply is itself soft, so the rows can't make the LP infeasible in practice). The loop sets `floor = ceil(Σ ceil(fc of env-E recipes) / machinesPerVaporizer)` — the WHOLE-building top-up over the fractional coverage the tie row already forces. `machinesPerVaporizer` (default `DEFAULT_MACHINES_PER_VAPORIZER = 4` in `src/lib/sustain-constants.ts` — UI-safe module, NOT calculator.ts, to keep the solver code-split) is a plan option (URL `mpv`) riding `calcProblem` for probe≡UI parity.
- **Fragmentation-aware cap tightening** (same loop): the soft facility-cap row bounds the FRACTIONAL count, but the game and the over-limit judge count PLACEMENTS (Σ ceil(x_r) per single-formula facility). When a capped facility's fractional total fits the cap but its placements exceed it (user-reported: Forge of the Sky 11.83 fractional / 13 placements vs cap 12), the loop tightens the LP row to `cap − (activeRecipes − 1)` (sufficient: Σ x_r ≤ cap − (R−1) ⟹ Σ ceil ≤ cap) and re-solves, letting the LP offload marginal production to alternative facilities before the warning fires. Single-formula facilities only (mix pools consolidate recipes per bin); fractionally-over caps are real demand pressure and stay with the slack path; `computeLimitViolations` always judges against the ORIGINAL user caps. Tightened caps only shrink (monotone, terminates).
- **Unified loop**: the power ceil-floor loop now converges four monotone whole-building figures per iteration — power floor, catalyst scales, vaporizer min-runs, tightened caps (`MAX_POWER_FLOOR_ITERATIONS = 8`). Vaporize bins render through the existing zero-output disposal flow (`disposal-<recipeId>` sinks); the merged mapper additionally draws pickup→sink edges for RAW-consuming disposal sinks (vaporizers burn raw gas — pre-1.4 no zero-output recipe consumed a raw directly).

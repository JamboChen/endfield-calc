...
- **Strict-equality demand**: `Σ_v u_v × rateDirection[r] = demand_r` per equivalence class and per pinned recipe (lines 929-955). Inequality (≥) would let variants over-produce one recipe to "fill" a bin, breaking Phase 2's flow balance.
...
- **Solver timeout**: `SOLVER_TIME_LIMIT_SECONDS = 30` (line 817), passed via `options: { timeLimitSeconds }` on the `LPModel` and translated by `highs-wrapper.ts:165` to HiGHS's native `time_limit` param — offset by the instance's accumulated run clock, which never resets (see `.claude/rules/solver.md`). Matches vitest's `testTimeout: 30000` (ms) in `vite.config.ts:92`.
...
- **Singleton fallback**: `emitSingletonBins` (line 1380) emits trivial 1-recipe bins for any active recipe the MIP didn't host. Always exact-match feasible per recipe because a singleton's direction is a unit vector. Triggered on LP infeasibility (line 1712) and for non-multi-formula recipes (no `cacheSlots`).
...
- **`aggregateBinTotals` (`plan-helpers.ts:164`) is the single source of truth** for buildings / power / per-facility totals. Per-recipe-ceiled aggregation triple-counts shared multi-formula bins. Source-facility power (pump_1/pump_2/unloader_1 from `rawMaterialSources`) is folded in here — do NOT re-sum at the caller.
